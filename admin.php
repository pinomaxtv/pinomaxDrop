...
if (isset($_GET['cashout_action']) && isset($_GET['cid'])) {
    ...
    if ($c_data && $c_data['status'] === 'pending') {
        // Kung REJECTED, awtomatikong ibalik ang pera sa balance ng user!
        if ($action === 'rejected') {
...
// FETCH ALL USERS WITH ASSET COUNT & TOTAL WITHDRAWN
$all_users = $pdo->query("
    SELECT u.*, 
...
// USER INSPECTOR DATA (Kapag nag-click ka ng specific user)
$view_user = null;
$view_files = [];
$view_cashouts = [];
...
<body class="bg-[#050608] text-gray-200 min-h-screen p-4 sm:p-8">
    ...
    <div class="max-w-6xl mx-auto space-y-8">
        <!-- TOP NAV -->
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#0a0c10] border border-red-500/30 p-6 rounded-3xl shadow-[0_0_30px_rgba(239,68,68,0.15)]">
            <div>
                <span class="text-[10px] font-black text-red-400 uppercase tracking-widest bg-red-500/10 border border-red-500/20 px-2.5 py-1 rounded-full">Super Admin Console</span>
                ...
                <p class="text-xs text-gray-500">Logged as <?= esc($admin['username']) ?> (<?= esc($admin['email']) ?>)</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <a href="dashboard.php" class="text-xs bg-[#00e5ff] text-black font-bold px-4 py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(0,229,255,0.3)]">Creator Hub</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div class="bg-[#0e1118] border border-white/5 p-4 rounded-2xl">
                <span class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Total Creators</span>
                <div class="text-xl sm:text-2xl font-black text-white mt-1"><?= number_format($total_users) ?></div>
            </div>
            <div class="bg-[#0e1118] border border-white/5 p-4 rounded-2xl">
                <span class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Uploaded Assets</span>
                <div class="text-xl sm:text-2xl font-black text-[#00e5ff] mt-1"><?= number_format($total_files) ?></div>
            </div>
            <div class="bg-[#0e1118] border border-white/5 p-4 rounded-2xl">
                <span class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Total Paid Out</span>
                <div class="text-xl sm:text-2xl font-black text-[#2ecc71] mt-1">₱<?= number_format($total_payouts, 2) ?></div>
            </div>
            <div class="bg-[#0e1118] border border-white/5 p-4 rounded-2xl">
                <span class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Users Unpaid Balances</span>
                <div class="text-xl sm:text-2xl font-black text-amber-400 mt-1">₱<?= number_format($total_system_balance, 2) ?></div>
            </div>
        </div>

        <!-- 🔍 USER INSPECTOR MODAL / CARD (KAPAG PININDOT ANG ISANG USER) -->
        <?php if ($view_user): ?>
        <div class="bg-[#0e1118] border-2 border-[#00e5ff]/40 rounded-3xl p-6 shadow-[0_0_50px_rgba(0,229,255,0.15)] relative">
            <div class="flex justify-between items-start border-b border-white/10 pb-4 mb-6">
                <div class="flex items-center gap-3">
                    <img src="<?= esc($view_user['avatar'] ?? 'https://ui-avatars.com/api/?name=User') ?>" class="w-12 h-12 rounded-2xl border border-white/10">
                    <div>
                        <span class="text-[10px] bg-[#00e5ff]/10 text-[#00e5ff] px-2 py-0.5 rounded font-bold uppercase">Inspecting User #<?= $view_user['id'] ?></span>
                        <h2 class="text-xl font-black text-white mt-0.5"><?= esc($view_user['username']) ?></h2>
                        <p class="text-xs text-gray-400"><?= esc($view_user['email']) ?></p>
                    </div>
                </div>
            ...
            <!-- EDIT FORM -->
            <form method="POST" class="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[#050608] border border-white/10 p-4 rounded-2xl mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="username" value="<?= esc($view_user['username']) ?>" required class="w-full bg-[#111318] border border-white/10 rounded-xl px-3 py-2 text-xs text-white">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Wallet Balance (₱)</label>
                    <input type="number" step="0.01" name="wallet_balance" value="<?= $view_user['wallet_balance'] ?>" required class="w-full bg-[#111318] border border-white/10 rounded-xl px-3 py-2 text-xs text-emerald-400 font-bold font-mono">
                </div>
            ...
            <div class="grid grid-cols-3 gap-3 mb-6 text-center text-xs">
                <div class="bg-white/5 p-3 rounded-xl">
                    <span class="text-gray-400 text-[10px]">Uploaded Assets</span>
                    <div class="font-bold text-white text-base mt-0.5"><?= count($view_files) ?></div>
                </div>
                <div class="bg-white/5 p-3 rounded-xl">
                    <span class="text-gray-400 text-[10px]">Total Cashed Out</span>
                    <div class="font-bold text-emerald-400 text-base mt-0.5">₱<?= number_format($view_user['total_paid'], 2) ?></div>
                </div>
                <div class="bg-white/5 p-3 rounded-xl">
                    <span class="text-gray-400 text-[10px]">Current Balance</span>
                    <div class="font-bold text-amber-400 text-base mt-0.5">₱<?= number_format($view_user['wallet_balance'], 2) ?></div>
                </div>
            </div>

            <!-- USER FILES & CASHOUTS TABS -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-[#0a0c10] border border-white/5 rounded-2xl p-4">
                    <h3 class="text-xs font-bold text-white uppercase mb-3 flex items-center gap-2">
                        <i class="fa-solid fa-folder text-[#00e5ff]"></i> Files by <?= esc($view_user['username']) ?> (<?= count($view_files) ?>)
                    </h3>
                    <div class="overflow-y-auto max-h-56 space-y-2">
                        ...
                        <div class="flex justify-between items-center bg-white/[0.02] p-2.5 rounded-xl text-xs">
                            <div>
                                <div class="font-bold text-white line-clamp-1"><?= esc($vf['title']) ?></div>
                                <div class="text-[10px] text-gray-500 font-mono"><?= number_format($vf['total_downloads']) ?> downloads • <?= esc($vf['file_size']) ?></div>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                <a href="admin.php?del_file=<?= $vf['id'] ?>&return_user=<?= $view_user['id'] ?>" onclick="return confirm('Burahin ang file?')" class="text-red-400 hover:text-red-300 font-bold text-[10px]">Delete</a>
                            </div>
                        </div>
                        <?php endforeach; else: ?>
...
<div class="text-[10px] text-amber-400 font-mono font-bold"><?= esc($vc['method']) ?></div>
                            ...
                            <div class="text-right">
                                ...
                                <?php if ($vc['status'] === 'pending'): ?>
                                    <div class="mt-1 space-x-1">
                                        <a href="admin.php?cashout_action=approve&cid=<?= $vc['id'] ?>&return_user=<?= $view_user['id'] ?>" class="text-[9px] bg-green-500 text-black px-1.5 py-0.5 rounded font-bold">PAID</a>
                                        <a href="admin.php?cashout_action=reject&cid=<?= $vc['id'] ?>&return_user=<?= $view_user['id'] ?>" class="text-[9px] bg-red-500 text-white px-1.5 py-0.5 rounded font-bold">REJECT</a>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <!-- 👥 ALL CREATORS & USERS TABLE (CLICK TO INSPECT) -->
        <div class="bg-[#0a0c10] border border-white/10 rounded-3xl p-6 shadow-xl">
            <h2 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <i class="fa-solid fa-users text-[#00e5ff]"></i> All Creators &amp; Users (Click to Manage)
            </h2>
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/[0.03] transition-colors <?= isset($_GET['view_user']) && $_GET['view_user'] == $u['id'] ? 'bg-[#00e5ff]/5' : '' ?>">
                            ...
                            <td class="p-3 text-center">
                                <a href="admin.php?view_user=<?= $u['id'] ?>" class="bg-[#00e5ff]/10 hover:bg-[#00e5ff] text-[#00e5ff] hover:text-black border border-[#00e5ff]/30 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all">
                                    🔍 Inspect / Edit
                                </a>
                            </td>
                        </tr>
        ...
        <!-- 💰 PENDING CASHOUT REQUESTS SECTION -->
        <div class="bg-[#0a0c10] border border-white/10 rounded-3xl p-6 shadow-xl">
            <h2 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <i class="fa-solid fa-money-bill-wave text-[#2ecc71]"></i> All Payout Transactions (GCash / PayPal)
            </h2>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs">
                    <thead class="bg-white/5 text-gray-400 uppercase font-bold text-[9px] tracking-wider">
                        ...
                        <tr class="hover:bg-white/[0.02]">
                            ...
                            <td class="p-3 text-right space-x-2">
                                <?php if ($c['status'] === 'pending'): ?>
                                    <a href="admin.php?cashout_action=approve&cid=<?= $c['id'] ?>" onclick="return confirm('Sigurado ka bang nabayaran mo na ito sa GCash/PayPal?')" class="bg-[#2ecc71] hover:bg-green-600 text-black font-black px-3 py-1 rounded-xl text-[10px]">MARK PAID</a>
                                    <a href="admin.php?cashout_action=reject&cid=<?= $c['id'] ?>" onclick="return confirm('I-reject at ibalik ang balance kay user?')" class="bg-red-600 hover:bg-red-700 text-white font-black px-3 py-1 rounded-xl text-[10px]">REJECT</a>
                                <?php else: ?>
